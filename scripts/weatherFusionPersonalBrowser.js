...
const fixture=()=>{
 const current={temperature:79,dewpoint:68,wind:4,windDirection:225,humidity:65,condition:'Cloudy',type:'observation',time:new Date(epoch-20*60000).toISOString(),pressurePa:101300,pressure:29.91,pressureTrend:{status:'ready',direction:'falling',deltaMb:-1.2,hours:3}};
 ...
 const hours=Array.from({length:48},(_,i)=>({time:new Date(Math.floor(epoch/H)*H+i*H).toISOString(),temperature:79+i*.08,condition:'Cloudy',pop:20,windDirection:'SW',windMph:4}));
...
try{
 for(const width of [320,360,390,514,768,1365]){
  ...
  assert.match(cloudyCard,/cloudy/i);assert.match(cloudyCard,/Warm outdoors/);
  ...
  assert.equal(await page.locator('#hourly .hour-wind').count(),await page.locator('#hourly .hour').count(),'Every hour must retain wind');
  assert.equal(await page.locator('#hourly .hour-uv').count(),await page.locator('#hourly .hour').count(),'Every hour must retain UV');
  assert.match(await firstHourly.locator('.hour-wind').innerText(),/4 mph[\s\S]*SW/);
  assert.equal(await page.locator('#hourly').evaluate(el=>el.textContent.includes('Station estimate')),false);
  assert.ok(await page.locator('.sun-shade-comparison').evaluate(el=>el.scrollWidth<=el.clientWidth+1),'All three people cards must fit together');
  const future=page.locator('#hourly .forecast-hour').first(),futureFeels=(await future.locator('.hour-feels b').innerText()).trim();
  await future.click();assert.equal((await page.locator('.sun-person figcaption strong').innerText()).trim(),futureFeels);assert.equal(await future.getAttribute('aria-pressed'),'true');
