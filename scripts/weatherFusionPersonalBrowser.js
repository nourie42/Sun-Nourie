import {CHANGES_VERSION,changeContext,validateChanges} from '../public/weather-fusion/forecast-changes.js';
...
app.get('/api/weather-fusion/:kind',(req,res)=>{
 ...
 if(req.params.kind==='briefing')return res.json({changesVersion:CHANGES_VERSION,forecastChanges:validateChanges([{candidateId:'change-0',summary:`Clouds lingering around ${f.location.name} could hold down afternoon temperatures.`}],changeContext(f,epoch),epoch),signature:f.signature,mode:'ai',generatedAt:f.assembledAt,headline:'Local outlook',summary:'Warm with rain possible.',uncertainty:`Rain timing could change around ${f.location.name}.`,nearTerm:'Clouds tonight.',extended:'A warmer week.',sources:['nws','afd']});
