...
try{
 for(const width of [320,360,390,514,768,1365]){
  ...
  assert.equal(await page.locator('.sun-shade-comparison figure').count(),2);assert.match(await page.locator('.sun-shade-comparison').innerText(),/84°/);assert.ok(!(await page.locator('#skin-exposure').innerText()).includes('~'));assert.ok(!(await page.locator('#skin-explanation').innerText()).includes('warmer than in shade'));
  ...
  report.checks.push({width,...layout,fullRanges:[24,48,168,240],sunShade:true,pressureMb:true,locationSwitch:true,dayGross:true,nwsAI:true});await context.close();
 }
