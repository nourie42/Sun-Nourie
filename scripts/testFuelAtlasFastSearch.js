...
function makeResponse() {
  return { statusCode: 200, headers: {}, payload: null, status(code) { this.statusCode = code; return this; }, setHeader(name, value) { this.headers[name.toLowerCase()] = value; }, json(value) { this.payload = value; return this; } };
}
...
function makeRoutes(options = {}) { const routes = new Map(); registerFuelAtlasFastSearchRoute({ get(path, handler) { routes.set(path, handler); } }, options); return routes; }

{
  ...
  const routes = makeRoutes({ fetchImpl, echoEndpoint: "https://echo.test/query", sourceTimeoutMs: 100, hardDeadlineMs: 200, googleSearches: [] });
...
{
  clearFuelAtlasFastCache();
  const routes = makeRoutes({ fetchImpl: async () => new Response("busy", { status: 503 }), echoEndpoint: "https://echo.test/query", sourceTimeoutMs: 60, hardDeadlineMs: 120, googleSearches: [] });
  const response = makeResponse();
  const started = Date.now();
...

{
  ...
  const fetchImpl = async (url) => {
    ...
    if (value.includes("maps.googleapis.com/maps/api/place/textsearch")) return jsonResponse({ status: "OK", results: [
  ...
  const routes = makeRoutes({ fetchImpl, googleApiKey: "test-key", echoEndpoint: "https://echo.test/query", sourceTimeoutMs: 100, hardDeadlineMs: 200, googleSearches: [{ query: "fuel oil supplier", type: "heating_oil" }] });
  const response = makeResponse();
  await routes.get("/api/fuel-atlas/search")({ query: { south: "39.8", west: "-75.3", north: "40.1", east: "-75.0", zoom: "11" } }, response);
