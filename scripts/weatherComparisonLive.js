...
const browser=await chromium.launch({headless:true});const report={deployed:true,liveData:true,point:point.id,runs:data.comparison.runs,viewports:[],panels:[]};
try{
 for(const width of [390,1280]){
  const context=await browser.newContext({viewport:{width,height:1000},reducedMotion:'reduce'}),page=await context.newPage();const errors=[];page.on('pageerror',e=>errors.push(e.message));
  ...
  await page.locator('button[data-view="both"]').click();await page.locator('button[data-section="today-forecast"]').click();
  // postMessage-driven scrolling and backdrop-filter compositing settle after the click.
  await page.waitForTimeout(1500);
  for(const source of ['fusion','google']){
   const frame=page.frames().find(f=>f.url().includes('source='+source));
   const bounds=await frame.locator('.today-weather-card').evaluate(el=>{const r=el.getBoundingClientRect();return {top:r.top,height:r.height,width:r.width};});
   report.panels.push({viewport:width,source,...bounds});
   await frame.locator('.today-weather-card').screenshot({path:`${dir}/${source}-today-${width}.png`});
  }
  await page.screenshot({path:`${dir}/live-${width}.png`,fullPage:false});
