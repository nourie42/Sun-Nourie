...
try{
 for(const width of [320,390,1440]){
  ...
  const dailyLayout=await page.evaluate(()=>{
   const row=document.querySelector('#daily .day-row'),rect=el=>el?.getBoundingClientRect(),rr=rect(row);
   const top=['.day-name','.day-icon','.day-low','.temp-track','.day-high'].map(s=>rect(row.querySelector(s))).filter(Boolean);
   const confidence=rect(row.querySelector('.forecast-confidence')),details=rect(row.querySelector('.day-feels-summary')),uv=rect(row.querySelector('.daily-uv'));
   const secondTops=[confidence.top,details.top,uv.top];
   return {
    rowFits:row.scrollWidth<=row.clientWidth+1,
    compactHeight:rr.height<=132,
    secondRowBelow:Math.min(...secondTops)>=Math.max(...top.map(r=>r.bottom))-3,
    secondRowAligned:Math.max(...secondTops)-Math.min(...secondTops)<5,
    confidenceLeft:confidence.left<details.left&&details.left<uv.left,
    noOverlap:confidence.right<=details.left+1&&details.right<=uv.left+1,
    leftMargin:confidence.left-rr.left>=6&&confidence.left-rr.left<=18,
    rightMargin:rr.right-uv.right>=6&&rr.right-uv.right<=18,
    allInside:[confidence,details,uv].every(r=>r.left>=rr.left&&r.right<=rr.right),
   };
  });
  if(width<=760)assert.deepEqual(dailyLayout,{rowFits:true,compactHeight:true,secondRowBelow:true,secondRowAligned:true,confidenceLeft:true,noOverlap:true,leftMargin:true,rightMargin:true,allInside:true});
  const placement=await page.evaluate(()=>{const q=s=>document.querySelector(s),a=q('.sun-person').getBoundingClientRect(),b=q('#pavement-content').getBoundingClientRect(),c=q('#skin-exposure').getBoundingClientRect();return {sameRow:Math.abs(a.top-b.top)<2,pawsRight:b.left>=a.right,inside:b.left>=c.left&&b.right<=c.right&&q('#skin-exposure').contains(q('#pavement-content')),three:q('.sun-shade-comparison').children.length,meta:[...document.querySelectorAll('.day-meta')].every(el=>{const f=el.querySelector('.forecast-confidence').getBoundingClientRect(),u=el.querySelector('.daily-uv').getBoundingClientRect();return f.left<u.left&&f.right<=u.left;})};});
  ...
  for(const [condition,kind,title] of [['Sunny','clear','Day'],['Mostly Cloudy','cloudy','Day'],['Partly Cloudy','partly-cloudy','Day']]){
   ...
   assert.equal(await page.locator('.sun-shade-comparison .reference-scene').count(),3);
   assert.equal(await page.locator('.shade-person .thermal-risk').count(),0);
   assert.equal(await page.locator('.shade-person .reference-scene').getAttribute('data-weather'),kind);
   assert.equal(await page.locator('.sun-person .reference-scene').getAttribute('data-weather'),kind);
   assert.ok(await page.locator('.sun-shade-comparison').evaluate(el=>el.scrollWidth<=el.clientWidth+1));
  ...
  const walker=await page.locator('.pavement-person .poodle-scene').getAttribute('data-outfit');assert.ok(['hot','warm'].includes(walker));
 ...
 assert.deepEqual(await page.locator('#outfits .reference-scene').evaluateAll(els=>els.map(e=>e.dataset.outfit)),['cold','cool','mild','warm','hot']);
 assert.equal(await page.locator('#outfits .reference-art').count(),5);
 report.scenarios.push('All five clothing states use the correct fixed reference artwork');
 ...
 await page.evaluate(async()=>Promise.all([...document.querySelectorAll('.poodle-scene .reference-art')].map(async el=>{const img=new Image();img.src=el.getAttribute('href');await img.decode();})));
 assert.deepEqual(await page.locator('.poodle-scene').evaluateAll(els=>els.map(e=>e.dataset.outfit)),['cold','cool','mild','warm','hot']);
