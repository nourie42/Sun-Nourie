...
try{
 for(const width of [320,360,390,430,768,1440]){
  ...
  await page.locator('#hour').fill('100');await page.locator('#hour').dispatchEvent('input');
  ...
  for(const path of ['/weather-fusion/','/weather-fusion/experimental-weather.html']){
   ...
   const nav=await shell.locator('.weather-jump-nav').evaluate(el=>({count:el.children.length,art:el.querySelectorAll('svg,img,picture,.jump-visual').length,overflow:el.scrollWidth>el.clientWidth+1,labels:[...el.querySelectorAll('.jump-label')].map(e=>e.textContent),heights:[...el.children].map(e=>e.getBoundingClientRect().height),fonts:[...el.querySelectorAll('.jump-label')].map(e=>parseFloat(getComputedStyle(e).fontSize))}));
   assert.equal(nav.count,5);assert.equal(nav.art,0);assert.equal(nav.overflow,false);assert.ok(nav.heights.every(v=>v>=44&&v<=50));assert.ok(nav.fonts.every(v=>v>=12));
 ...
 report.success=true;report.checks=['All 21 field selectors and six ensemble statistics','360-hour main and separate 48-hour interim runs','Hour slider and daily/full-horizon navigation','Location selection','CSV actual download and 360 records','Refresh failure retains original values and timestamps','First-load failure displays no invented forecast','Five readable image-free buttons at all six widths','Experimental-only link and no embedded WeatherNext card','Production static route allowlist','No horizontal overflow or JavaScript errors'];
