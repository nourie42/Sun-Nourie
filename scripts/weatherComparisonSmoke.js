...
try{
 ...
 for(const width of [320,390,768,1280]){
  ...
  await p.waitForFunction(()=>document.querySelector('#compare-status').textContent.includes('Both forecasts loaded'),{timeout:45000});
  ...
  await p.route('**/api/weather-fusion/compare/google*',r=>r.fulfill({status:503,contentType:'application/json',body:JSON.stringify({error:'Test refresh failure'})}));const previous=await g.locator('#temperature').innerText();await p.locator('#compare-refresh').click();await p.waitForFunction(()=>document.querySelector('#compare-status').textContent.includes('Google:'),{timeout:15000});assert.equal(await g.locator('#temperature').innerText(),previous);
