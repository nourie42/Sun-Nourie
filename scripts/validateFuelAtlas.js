...
const selected = String(process.argv[2] || "all").toLowerCase();
if (selected === "all") {
  for (const validate of Object.values(groups)) validate();
} else {
  assert.ok(groups[selected], `Unknown Fuel Atlas validation group: ${selected}`);
  groups[selected]();
}

console.log(`Fuel Atlas validation passed: ${selected}.`);
