...
export function restoreSourceQuotes(raw:any,sources:any[]){
 ...
 for(const e of Array.isArray(raw?.evidence)?raw.evidence:[]){
  if(!Number.isFinite(e.value)||typeof e.quote!=='string')continue;
  const original=sources.find(s=>s.id===e.sourceId);if(original&&norm(original.text).includes(norm(e.quote)))continue;
  const field=fields.find(f=>f.key===e.field);if(!field)continue;
  ...
  for(const source of sources)for(const line of String(source.text||'').split(/\r?\n/)){
   if(line.length>2500||!numberSupported(e.value,line,String(e.sourceUnit||''),e.field))continue;
   const score=words.filter(w=>norm(line).includes(w)).length;if(score<1)continue;
...
export function normalizeReview(raw:any, sources:any[], current:any, verification:any, period:string){
 ...
 for(const key of ['gaSavings','cardSavings','maintenanceSavings','otherSavings','exitRecovery','customSynergies'])if(currentDeal[key]!==undefined)deal[key]=currentDeal[key];
...
export function derivedSupported(e:any,value:number,sources:any[],key:string){
 ...
 const v=cs.map(c=>c.value);const expected=e.operation==='ttm'&&v.length===3?v[0]+v[1]-v[2]:e.operation==='sum'?v.reduce((a,b)=>a+b,0):e.operation==='difference'?v[0]-v[1]:e.operation==='ratioCpg'&&v.length===2&&v[1]>0?v[0]/v[1]*100:NaN;
