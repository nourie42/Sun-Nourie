...
export function arkoAnnualTables(sources,period){
 ...
 if(!source||period!=='FY2025')return null;
 try{
  ...
  function metric(start,label,unit='USD thousands'){
   ...
   const value=values[2]*(unit.includes('thousands')?1000:1);
   return {value,sourceId:source.id,quote:lines[i],sourceUnit:unit,period,locator:`${lines[start]} · year ended December 31, 2025 · ${label}`,status:'supported',reason:'Read directly from the issuer table: validated annual-year column, original unit and row label.'};
  }
