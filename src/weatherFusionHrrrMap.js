/** NOAA HRRR low-level REFD published by Iowa State, with pinned initialization.
 * Primary provider docs: https://mesonet.agron.iastate.edu/GIS/model.phtml . */
...
export function hrrrTileManifest(meta,now=Date.now(),checkedAt=new Date(now).toISOString()) {
 ...
 const frames=Array.from({length:19},(_,hour)=>({
  ...
  field:'reflectivity',product:'REFD_1000m_AGL',units:'dBZ',kind:'xyz',bounds:[[20,-130],[55,-60]],
...
export function createHrrrMapSource({fetchImpl=globalThis.fetch,now=Date.now}={}) {
 ...
 return async()=>{
  if(until>now())return cache;if(pending)return pending;
  pending=(async()=>{
   try {
   ...
   }catch{cache=cache?.layer&&now()-Date.parse(cache.layer.runAt)<4*H?{...cache,status:'check-failed'}:{layer:null,status:'unavailable'};}
