...
export async function extractChannels({ask,content,sources,review,period}){
 const raw=parseAnalysis(await ask([{role:'user',content:[{type:'text',text:`Extract ALL non-retail operating channels for this company and ${period}. Company-operated retail is already modeled separately. Do not count company-operated locations again. Existing dealers may be part of wholesale: create a single channel for overlapping dealer/wholesale locations, not two. Separate fleet/cardlock only if disclosed separately. Corporate G&A belongs once: return corporateGa not repeated segment G&A. Return JSON {channels:[{name,type:"dealer|wholesale|fleet|other",metrics:{sites,gallons,fuelCpg,other,opex,ga,capex}}],corporateGa:null}. EACH metric is null or {value,quote,sourceId,sourceUnit,period,locator}; use exact short numeric passages from source tables. For TTM derive only full prior year + current YTD − comparable prior YTD, with operation:"ttm" and components:[{value,quote,sourceId,sourceUnit,period}], otherwise null. Values in USD, gallons, cents/gallon or counts, fully scaled. Include reported channel other GP/rental income. Operating costs exclude D&A, interest and corporate G&A; if combined with D&A leave unavailable. No inventing addresses. Do not exclude channels just because financial values are missing. JSON only, compact.`},...content]}],{max_tokens:7000}));
 for(const c of raw.channels||[])for(const [field,m] of Object.entries(c.metrics||{})){if(m){m.field=({opex:'sellerOpex',ga:'sellerGa',capex:'maintenanceCapex'})[field]||field;restoreSourceQuotes({evidence:[m]},sources);}}if(raw.corporateGa){raw.corporateGa.field='sellerGa';restoreSourceQuotes({evidence:[raw.corporateGa]},sources);}
 const evidence=[];
 ...
 const channels=(raw.channels||[]).slice(0,8).map((c,i)=>{
  ...
  for(const k of ['sites','gallons','fuelCpg','other','opex','ga','capex']){
   ...
   result[k]=ok?m.value:null;result.metricEvidence[k]={...(ok?m:{}),status:ok?(m.operation?'Calculated from reported figures':'Reported'):'Estimated',reason:ok?(m.operation?'Calculated TTM = prior full year + current YTD − comparable prior YTD; verified component sources.':'Verified source table and channel perimeter.'):'Not available in verified source tables; channel assumption shown.'};
  }
