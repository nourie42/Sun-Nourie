...
export async function extractChannels({ask,content,sources,review,period}){
 ...
 const channels=(raw.channels||[]).slice(0,8).map((c,i)=>{
  ...
  for(const k of ['sites','gallons','fuelCpg','other','opex','ga','capex']){
   ...
   result[k]=ok?m.value:null;result.metricEvidence[k]={...(ok?m:{}),status:ok?'Reported':'Estimated',reason:ok?'Verified source table and channel perimeter.':'Not available in verified source tables; channel assumption shown.'};
  }
