...
export function buildForecast({ location, point, forecast, hourly, grid, discussion, observation, alerts, specialDiscussions, precipitationDiscussions, riskOutlooks, exposureWeather, models, feeds, now }) {
  ...
  const output = { version: VERSION, assembledAt: iso(now), location: { ...location, name: resolvedName, timeZone: zone, office: point?.cwa || null },
    current, hours, days, discussion: discussion || null, alerts: alerts || [], specialDiscussions: specialDiscussions || [], precipitationDiscussions: precipitationDiscussions || [], riskOutlooks: riskOutlooks || [], feeds,
  ...
  async function getForecast(query) {
    ...
    const snapshot=await forecastCache.get(key, MINUTE, async () => {
      ...
      const jobs = {
        ...
        precipitationDiscussions: loadPrecipitationDiscussions(location),
