...
function nwsOnlyRainLikelihood(likelihood) {
  const nws=likelihood?.officialProbability;
  if(!finite(nws))return likelihood;
  const sourcePoints={nws,hrrr:null,ecmwf:null,nbm:null};
  return {...likelihood,value:Math.round(nws),rawValue:Math.round(nws),weightedValue:nws,rawTotal:nws,
    sourcePoints,sources:[{id:'nws',value:nws,weight:1,points:nws,runAt:likelihood.sources?.find(source=>source.id==='nws')?.runAt||null}],
    wetSources:[],reducedSources:[],source:'NWS probability; deterministic model precipitation was not sustained across adjacent hours',
    fallbackReason:'no-sustained-model-precipitation'};
}
...
export function enhanceForecast(out, { models, grid, periods = [], hourlyPeriods = [], now, gridQpf, localTime, nextDate, dateKey }) {
  ...
  out.rainTimeline=out.rainTimeline.map((row,index)=>forecastDayIndex(Date.parse(row.time),now,out.location.timeZone)>0&&!extendedWet[index]
    ? {...row,rainLikelihood:nwsOnlyRainLikelihood(row.rainLikelihood)}:row);
