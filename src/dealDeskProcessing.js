...
export async function verifyBoundedAnalysis({ask,content,raw,period,phase}){
 const request=async proposal=>{
  const value=parseAnalysis(await ask([{role:'user',content:[{type:'text',text:`Independently verify this proposed company analysis against the supplied sources. Return compact JSON only {approvedFields:[],companySupported:boolean,summarySupported:boolean,rejected:[{field,reason}]}. approvedFields MUST contain only exact internal field-key strings, such as "sites" or "gallons", never labels or objects. Allowed keys: ${fields.map(f=>f.key).join(', ')}. Every proposed numeric field must be either approved or rejected with a specific reason. At most one rejection per field, reason at most 120 characters. Do not repeat source text, site rows or the proposal. Approve a field ONLY if its exact value, original unit/scale, period, currency USD and acquired perimeter are explicit, the quote matches, the field meaning/cost responsibility is correct, and no source conflicts. Reject assumptions, scanned/image values without reliable text, unsupported annualization, wrong periods, and prefilled legacy examples. Approve transparent arithmetic from reported operands when each component source, period, unit and perimeter is explicit: sum/difference, fuel margin cents = reported fuel GP USD / reported gallons * 100, and TTM = prior full year + current YTD − comparable prior YTD. For a YTD request approve reported YTD values as-is; the application annualizes separately. TTM operands must cover exactly 12 non-overlapping months. Derived values must have evidence components; never invent operands. Cached workbook formula results explicitly reported in the source are source-reported values, not newly derived assumptions. Accounting whitespace between a currency symbol and its number is immaterial. Different reporting periods are not conflicting values: verify only the chosen annual period. Unit conversion from explicitly reported USD/gallon to cents/gallon is allowed. Historical purchase costs do not establish proposed acquisition consideration. Approve company/summary only when all facts are supported, hypotheses clearly labeled, and numbers do not imply unapproved synergies. Proposed analysis: ${JSON.stringify(proposal)}\nSelected period: ${period||'one common reported annual period'}`},...content]}],{max_tokens:6000}));
