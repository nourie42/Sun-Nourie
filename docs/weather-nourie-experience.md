...
Main feels-like numbers (hero, Now, hourly forecast, metric, graphs, daily extrema and outdoor figure) use the same outdoors exposure. Shade is retained as a separately labeled comparison. Each forecast hour uses its own temperature, moisture, wind, sky and solar time. The raw thermal series is never raised simply to match another exposure, and the thermal equations are unchanged. The UI applies one explicit display-only sanity rule to the primary outdoor feels-like: when same-hour dew point is available and forecast gusts are below 25 mph, the shown feels-like is not allowed below that dew point. The raw UTCI/Tier-3 value remains available for integrity checks and scientific details. API `metricForecasts.series.feels` is outdoors; `feelsShade` retains shade, and `outdoorFeelsVersion` identifies this contract. Legacy `current.apparent` remains the named Steadman diagnostic; UI consumers use `current.feelsLike` / `comfort.outdoors`.
