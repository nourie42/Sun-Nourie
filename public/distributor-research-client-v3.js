(() => {
  ...
  const ACTIVE_JOB_KEY = 'fuelIqActiveDistributorResearchJobV3';
  let activeJobId = '';
  let polling = false;
  let stopped = false;
  let networkFailures = 0;

  // The product now offers a single, reliable Word export.
  ...
  try {
    localStorage.removeItem('fuelIqActiveDistributorResearchJobV2');
  } catch {}
  ...
  function statusMessage(data) {
    ...
    const phase = data?.phase === 'formatting' ? ' • formatting report' : ' • researching sources';
    const attempt = data?.maxAttempts > 1 ? ` • attempt ${data.attempt || 1}/${data.maxAttempts}` : '';
    const model = data?.model ? ` • ${data.model}` : '';
    return `${data?.message || 'OpenAI is researching public sources…'}${time}${phase}${model}${attempt}`;
  }
  ...
  async function startResearch(event) {
    ...
    stopped = false;
    networkFailures = 0;
    setUiLoading('Starting public-source research…');
    ...
    try {
      ...
      showFailure(error?.name === 'AbortError'
        ? 'The research job could not be started before the server timeout.'
        : (error?.message || String(error)));
    }
  }

  // Capture-phase interception prevents the page's legacy long-lived request from running.
  // The company picker registers first and still enforces selecting a company.
