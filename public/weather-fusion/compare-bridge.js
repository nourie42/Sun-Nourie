...
export function installComparisonPane(config,actions){
 ...
 function googleLabels(data){
  ...
  for(const button of document.querySelectorAll('#hourly [data-comfort-time]')){
   ...
   if(rain){rain.title='Model mean precipitation amount for this hour, not rain probability or observed radar.';rain.innerHTML=`<small class="hour-pop">${typeof row?.precipitation==='number'?row.precipitation.toFixed(2)+' in':'— in'}</small>`;}
  }
