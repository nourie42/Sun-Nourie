...
import {thermalComfort, finite, solarElevation} from './weather-math.js?v=evidence-v1';
import {feelsAt, forecastValue, degrees} from './hourly-feels.js?v=evidence-v1';
...
export function hourlyDisplaySamples(forecast, now = Date.now()) {
  return [currentSample(forecast, now), ...(forecast?.hours || []).filter(hour => Date.parse(hour.time)+3600000 > now)
