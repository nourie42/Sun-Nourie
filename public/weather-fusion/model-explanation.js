...
const WEATHERNEXT_ACCESS = 'https://developers.google.com/weathernext/guides/access-forecast';
...
let weatherNextState = {key:null,status:'idle',point:null,runAt:null,generatedAt:null,checkedAt:0,message:''};
...
function stamp(value, zone, includeDate = true) {
  ...
  return new Intl.DateTimeFormat('en-US',{timeZone:zone,...(includeDate?{weekday:'short',month:'short',day:'numeric'}:{}),hour:'numeric',minute:'2-digit'}).format(new Date(epoch)).replace(':00','');
}
...
function number(value, precision = 4) { return finite(value) ? Number(value.toFixed(precision)).toString() : '—'; }
...
function weatherNextKey(forecast) {
...
function weatherNextPoint(payload, forecast) {
...
function weatherNextHours(point) {
  return (Array.isArray(point?.hourly)?point.hourly:[]).flatMap(row=>{
    const time=Date.parse(row.time);
    if(!finite(time))return [];
    return [{time,
      temperature:finite(row.temperatureF)?row.temperatureF:null,
      p10:finite(row.temperatureP10F)?row.temperatureP10F:null,
      p90:finite(row.temperatureP90F)?row.temperatureP90F:null,
      wind:finite(row.windMph)?Math.max(0,row.windMph):null,
      rain:finite(row.precipitationInches)?Math.max(0,row.precipitationInches):null,
...
function queueWeatherNextLoad(forecast) {
  ...
  const key=weatherNextKey(forecast);if(!key)return;
  const retry=Date.now()-(weatherNextState.checkedAt||0)>5*60000;
  if(weatherNextState.key===key&&(weatherNextState.status==='loading'||(!retry&&weatherNextState.status!=='idle')))return;
  weatherNextState={key,status:'loading',point:null,runAt:null,generatedAt:null,checkedAt:Date.now(),message:''};
  fetch(WEATHERNEXT_FEED,{cache:'no-store',headers:{Accept:'application/json'}}).then(async response=>{
    if(!response.ok)throw new Error(`HTTP ${response.status}`);
    const payload=await response.json();
    if(payload?.schema!=='weather-nourie-weathernext3-v1')throw new Error('WeatherNext feed schema mismatch');
    const point=weatherNextPoint(payload,forecast);
    weatherNextState={key,status:point?'ready':'not-covered',point,runAt:payload?.runAt||null,generatedAt:payload?.generatedAt||null,checkedAt:Date.now(),message:''};
  }).catch(error=>{
...
function installWeatherNextMapControl() {
  if(typeof document==='undefined'||!experimentalPath())return;
  const tabs=document.querySelector?.('.map-tabs');
  if(!tabs)return;
  const old=tabs.querySelector?.('[data-graphcast-map]');if(old)old.remove();
  if(tabs.querySelector?.('[data-weathernext-map]'))return;
  const button=document.createElement('button');
  button.type='button';button.disabled=true;button.className='graphcast-map-pending';button.dataset.weathernextMap='pending';
  button.textContent='WeatherNext 3';button.title='WeatherNext 3 map activates after an authorized hosted map feed is configured.';
  button.setAttribute('aria-label','WeatherNext 3 map pending data access');
  tabs.append(button);
}
...
if(typeof document!=='undefined')installWeatherNextMapControl();
