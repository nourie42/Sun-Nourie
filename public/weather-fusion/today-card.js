...
export function todayForecastHTML(forecast,now=Date.now()){
 ...
 const hrrrRain=hrrrRainAmount(day),hrrrText=finite(hrrrRain)?`HRRR: rain ${p.tonight?'tonight':'today'}`:'';
 ...
 <span class="today-symbol">${weatherIcon(p.condition,!p.tonight,80)}<strong>${reading(p.pop,'%')}</strong><small>NWS chance</small>${hrrrText?`<em title="HRRR models ${hrrrRain<.01?'less than 0.01':hrrrRain.toFixed(2)} inches for this period">${hrrrText}</em>`:''}</span>
