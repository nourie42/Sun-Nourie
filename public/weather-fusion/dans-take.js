 ...
 */
export const DAN_TAKE_VERSION = 'weather-nourie-dans-take-v3';
...
function acceptableParaphrase(text, candidate) {
  ...
  if(/\b(yesterday|last night|earlier today|today|tonight|tomorrow)\b/i.test(text))return false; // never ambiguous relative dates
  ...
  // Context is part of the exact excerpt, but yesterday's recap cannot
...
export function approveDanTake(proposals, data, now=Date.now()) {
  ...
  for(const proposal of Array.isArray(proposals)?proposals:[]){
    ...
    if(!c||seen.has(c.id)||!acceptableParaphrase(proposal.summary,c)){rejected.push({evidenceId:String(proposal?.evidenceId||'').slice(0,60),reason:!c?'unrecognized-evidence':seen.has(c.id)?'duplicate':'paraphrase-validation'});continue;}
