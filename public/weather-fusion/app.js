...
function renderMetrics(data) {
  ...
  $('metrics').innerHTML = [
    card('FEELS LIKE', 'temp', temperature(c.apparent), 'Model-derived apparent temperature; not an observed measurement.'),
    card('PRECIPITATION', 'drop', finite(d.qpf) ? `${number(d.qpf, 2)}<small>in</small>` : '—', `7 AM–7 AM · ${d.qpfSource}. Full-window liquid equivalent.`),
    ...
    card('SUNSET', 'sun', data.solar.sunset ? esc(clock(data.solar.sunset)) : '—', data.solar.sunrise ? `Sunrise ${clock(data.solar.sunrise)} · local time.` : 'Sunrise and sunset appear when the model data connection is configured.', data.solar.sunset ? '<div class="sun-arc" aria-hidden="true"></div>' : '', 'metric-sun'),
  ].join('');
}
function renderEvidence(data) {
  ...
  const names = { ready: 'Available', unavailable: 'Unavailable', stale: 'Stale', 'not-configured': 'Setup required' };
  $('feed-health').innerHTML = important.map((id) => {
    const f = data.feeds.find((s) => s.id === id);
    return `<span class="feed-chip ${esc(f?.status || 'unavailable')}" title="${esc(f?.message || f?.label || '')}">${labels[id]} · ${names[f?.status] || 'Unavailable'}</span>`;
  }).join('');
  ...
  $('source-register').innerHTML = data.feeds.map((f) => {
    const url = /^https:\/\/(api\.weather\.gov|(?:customer-)?api\.open-meteo\.com|open-meteo\.com)\//.test(f.url || '') ? f.url : 'https://www.weather.gov/';
...
function renderBriefing(data) {
  ...
  $('ai-label').textContent = data.mode === 'ai' ? 'AI + NWS DISCUSSION' : 'OFFICIAL NWS TEXT';
  const refs = (data.sources || []).filter((id) => ['nws', 'afd', 'hrrr', 'ecmwf', 'nbm'].includes(id)).map((id) => {
    ...
    return url && /^https:\/\/(api\.weather\.gov|(?:customer-)?api\.open-meteo\.com)\//.test(url) ? `<a href="${esc(url)}" target="_blank" rel="noopener noreferrer">${esc(f?.label || id)} ↗</a>` : esc(f?.label || id);
  });
...
async function load({ moveMap = false } = {}) {
  ...
  try {
    ...
    if (moveMap && selectedLayer !== 'radar') loadModelMap();
...
function showDay(index) {
  ...
  if (!d) return;
  const modelNames = { hrrr: 'HRRR', ecmwf: 'ECMWF IFS HRES', nbm: 'NOAA National Blend' };
  const modelRows = Object.entries(d.guidance).map(([id, v]) => `<tr><td>${modelNames[id]}</td><td>${temperature(v.high)} / ${temperature(v.low)}</td><td>${inches(v.qpf)}</td><td>${finite(v.gust) ? `${number(v.gust)} mph` : '—'}</td></tr>`).join('');
  $('day-content').innerHTML = `<div class="dialog-eyebrow">${esc(d.date)} · NWS ${esc(forecast.location.office || 'FORECAST')}</div><h2 id="day-title" class="dialog-title">${esc(d.label === 'Today' ? 'Today & tonight' : d.label)}</h2><p class="dialog-condition">${esc(d.condition)}</p><div class="dialog-temps">${temperature(d.high)}<span>${temperature(d.low)}</span></div><div class="dialog-stats"><div><strong>${percent(d.popDay)} / ${percent(d.popNight)}</strong><small>NWS chance · day / night</small></div><div><strong>${inches(d.qpf)}</strong><small>Full 7 AM–7 AM liquid-equivalent forecast</small></div></div><p class="dialog-prose">${esc(d.detail || 'Official forecast details are unavailable.')}</p>${d.nightDetail && d.nightDetail !== d.detail ? `<p class="dialog-prose"><strong>Tonight:</strong> ${esc(d.nightDetail)}</p>` : ''}<h3 class="dialog-subtitle">Source comparison · ${esc(d.agreement)}</h3><table class="comparison"><thead><tr><th>Source</th><th>High / low</th><th>Precipitation</th><th>Peak gust</th></tr></thead><tbody><tr><td>NWS / primary display</td><td>${temperature(d.high)} / ${temperature(d.low)}</td><td>${inches(d.qpf)}</td><td>—</td></tr>${modelRows}</tbody></table><p class="table-note">Primary precipitation: ${esc(d.qpfSource)}. Window: ${esc(clock(d.qpfWindow.start, { month: 'short', day: 'numeric' }))} → ${esc(clock(d.qpfWindow.end, { month: 'short', day: 'numeric' }))}. Includes earlier forecast hours; it is not measured accumulation.</p><p class="table-note">Model temperatures use complete calendar days; NWS lows apply overnight. ${esc(d.temperatureSource)}. Missing full-window model coverage is shown as unavailable, not zero. Gust guidance can cover only part of a day.</p>${d.illustrativeBlend?.value != null ? `<p class="table-note">Illustrative model-only precipitation blend: <strong>${inches(d.illustrativeBlend.value)}</strong> · ${d.illustrativeBlend.sources.map((s) => `${esc(s.name)} ${Math.round(s.weight * 100)}%`).join(' / ')}. This heuristic has not been calibrated against local observations and does not replace NWS guidance.</p>` : ''}<p class="table-note">NWS rain probability is separate from precipitation amount. Model disagreement is not an ensemble confidence interval.</p>`;
...
async function loadRadar() {
  ...
  try {
    ...
    if (!frames.length) { stopRadar(); if (radarLayer) { map?.removeLayer(radarLayer); radarLayer = null; } $('radar-time').disabled = true; $('radar-play').disabled = true; $('radar-stamp').textContent = 'Unavailable'; if (selectedLayer === 'radar') mapMessage(data.message); return; }
    frameIndex = frames.length - 1;
    $('radar-time').max = String(frameIndex); $('radar-time').value = String(frameIndex); $('radar-time').disabled = false;
    $('radar-play').disabled = frames.length < 2;
    if (selectedLayer === 'radar') { showFrame(frameIndex); mapMessage(data.status === 'stale' ? 'The latest advertised radar frame is more than 20 minutes old.' : ''); }
  } catch { if (id === radarGeneration && selectedLayer === 'radar') { stopRadar(); mapMessage('Radar timestamps could not be checked. Any displayed image may be stale.'); $('radar-stamp').textContent = 'Check failed'; } }
}
function showFrame(index) {
  if (!map || !window.L || !radarMeta || !frames[index]) return;
  frameIndex = index;
  if (radarLayer) map.removeLayer(radarLayer);
  const expected = frames[index];
  radarLayer = window.L.tileLayer.wms(radarMeta.url, { layers: radarMeta.layer, format: 'image/png', transparent: true, version: '1.1.1', opacity: .72, time: expected, zIndex: 200, attribution: 'Radar © NOAA / NWS', updateWhenIdle: true }).addTo(map);
  $('radar-time').value = String(index); $('radar-stamp').textContent = `${clock(expected)} · loading`;
  radarLayer.on('load', () => { if (frames[frameIndex] === expected) $('radar-stamp').textContent = clock(expected); });
  radarLayer.on('tileerror', () => { if (selectedLayer === 'radar') { stopRadar(); mapMessage('Some radar tiles failed to load; do not interpret blank areas as clear weather.'); } });
}
function stopRadar() { if (radarTimer) clearInterval(radarTimer); radarTimer = null; $('radar-play').textContent = '▶'; $('radar-play').setAttribute('aria-label', 'Play radar animation'); }
function loadModelMap() {
  const p = new URLSearchParams({ lat: place.latitude, lon: place.longitude, detailLat: place.latitude, detailLon: place.longitude, zoom: '7', level: 'surface', overlay: selectedLayer, product: 'ecmwf', type: 'map', location: 'coordinates', metricWind: 'mph', metricTemp: '°F', metricRain: 'in', marker: 'true', calendar: 'now' });
  $('model-map').src = `https://embed.windy.com/embed2.html?${p}`;
  $('map-source').href = `https://www.windy.com/?${selectedLayer},${place.latitude},${place.longitude},7`;
}
function selectLayer(layer) {
  selectedLayer = layer; stopRadar();
  ...
  $('radar-map').hidden = !radar; $('model-map').hidden = radar; $('radar-controls').hidden = !radar; $('radar-legend').hidden = !radar;
  mapMessage('');
  $('map-caption').textContent = radar ? 'NOAA observed reflectivity · selected-point alert polygons' : 'Windy / ECMWF model guidance · not the local NWS forecast or radar';
  $('map-source').textContent = radar ? 'Official radar ↗' : 'Open map provider ↗';
  if (radar) { $('model-map').removeAttribute('src'); $('map-source').href = 'https://radar.weather.gov/'; map?.invalidateSize(); showFrame(frameIndex); if (!frames.length) void loadRadar(); }
  else { if (radarLayer) { map?.removeLayer(radarLayer); radarLayer = null; } loadModelMap(); }
}
...
$('radar-time').addEventListener('input', () => { stopRadar(); showFrame(Number($('radar-time').value)); });
$('radar-play').addEventListener('click', () => {
  ...
  radarTimer = setInterval(() => showFrame((frameIndex + 1) % frames.length), 1200);
});
