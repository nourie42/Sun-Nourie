...
import {weatherIcon,renderHourlyWeather,currentSample,heroFeelsHTML} from './weather-display.js?v=sun-exposure-v1';
...
function render(data) {
  ...
  const sample=currentSample(data),sunDisplay=sunExposureTemperature(sample);
  ...
  $('temperature').innerHTML = `${number(sunDisplay.active?sunDisplay.value:hero.temperature)}<span>°</span>`;
  document.querySelector('.current-temp-label').textContent=sunDisplay.active?(sample.comfort.weatherKind==='partly-cloudy'?'Sunny-break estimate':'In direct sun · estimated'):'Current air temperature';
  if($('hero-feels'))$('hero-feels').innerHTML=heroFeelsHTML(sample,{sunExposure:sunDisplay.active});
  ...
  $('observation-label').textContent = `${sourceLabel}${sunDisplay.active?` · air ${number(sample.temperature)}° · modeled sun effect +${number(sunDisplay.solarLift)}°`:''}${radarLabel?` · ${radarLabel}`:''}`;
