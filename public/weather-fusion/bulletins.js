import {bulletinFacts,BULLETIN_GROUPS} from './bulletin-facts.js?v=wpc-mpd-v1';
...
export function renderBulletins(forecast,result=null,now=Date.now()){
 ...
 const precipitationDiscussionStatus=forecast.feeds?.find(f=>f.id==='precipitation-discussions')?.status;
 ...
 root.querySelectorAll('[data-bulletin-index]').forEach(button=>button.addEventListener('click',()=>{
  ...
  const aiSummary=summaries.find(s=>s.id===item.id&&s.sourceKey===item.sourceKey),summary=aiSummary?.summary||item.plainSummary;
  const official=item.kind==='discussion'?`<details class="bulletin-official-details"><summary>Show full official discussion</summary><pre class="bulletin-dialog-wording">${esc(item.description)}</pre></details>`:`<h3 class="dialog-subtitle">Complete official wording</h3>${item.headline?`<p class="dialog-prose">${esc(item.headline)}</p>`:''}<pre class="bulletin-dialog-wording">${esc(item.description)}</pre>`;
  content.innerHTML=`<div class="dialog-eyebrow">${esc(BULLETIN_GROUPS[item.kind]||'NWS bulletin')}</div><h2 id="bulletin-dialog-title" class="dialog-title">${esc(item.title)}</h2><p class="dialog-condition">${esc(item.area)}</p><p class="bulletin-dialog-time">${item.kind==='discussion'?'Valid from':'Issued'} ${esc(time(item.issuedAt))}${item.expires?` · expires ${esc(time(item.expires))}`:''}</p>${summary?`<div class="bulletin-dialog-summary"><strong>What it means</strong><p>${esc(summary)}</p></div>`:''}${item.instruction?`<div class="bulletin-dialog-instruction"><strong>Official instructions</strong><p>${esc(item.instruction)}</p></div>`:''}${official}<a class="bulletin-dialog-source" href="${esc(item.url)}" target="_blank" rel="noopener noreferrer">Open original NWS bulletin ↗</a>`;
