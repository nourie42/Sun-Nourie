import {bulletinFacts,BULLETIN_GROUPS} from './bulletin-facts.js';
...
export function renderBulletins(forecast,result=null,now=Date.now()){
 ...
 const discussionStatus=forecast.feeds?.find(f=>f.id==='afd')?.status;
 ...
 const notes=(status!=='ready'?'<p class="bulletin-unavailable">Live alert status is unavailable or stale. An empty list does not establish that there are no warnings. Check the NWS.</p>':'')+(discussionStatus!=='ready'?'<p class="bulletin-unavailable">The local NWS discussion is unavailable or stale.</p>':'');
 root.innerHTML=`${counts?`<div class="bulletin-counts">${counts}</div>`:''}${notes}${items.map(item=>{
  ...
  const fallback=item.kind==='discussion'?(result?'AI summary unavailable. The complete official discussion is below.':'Preparing a plain-language explanation of the regional discussion…'):item.description.slice(0,650)+(item.description.length>650?'…':'');
  return `<article class="bulletin-item bulletin-${item.kind}"><h3>${esc(item.title)}</h3><p class="bulletin-area">${esc(item.area)}</p><p class="bulletin-time">Issued ${esc(time(item.issuedAt))}${item.expires?` · expires ${esc(time(item.expires))}`:''}</p><span class="bulletin-mode">${summary?'AI plain-language summary':item.kind==='discussion'?'NWS regional discussion':'Official NWS wording'}</span><p class="bulletin-summary">${esc(summary?.summary||fallback)}</p>${item.instruction?`<p class="bulletin-instruction"><strong>Official instructions:</strong> ${esc(item.instruction)}</p>`:''}<details data-bulletin-id="${esc(item.id)}"${open.has(item.id)?' open':''}><summary>Read the complete official bulletin</summary>${item.headline?`<p>${esc(item.headline)}</p>`:''}<pre>${esc(item.description)}</pre>${item.instruction?`<p>${esc(item.instruction)}</p>`:''}</details><a href="${esc(item.url)}" target="_blank" rel="noopener noreferrer">Original NWS source ↗</a></article>`;
