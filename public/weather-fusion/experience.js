...
export function renderMetricTiles(forecast,smallIcon) {
 ...
 const pressureSeries=pointsFor('pressure',24),futurePressure=pressureSeries.find(p=>Date.parse(p.time)>now+2.5*3600000),currentPressure=stationPressureMb(c);
 const pressureNote=c.pressureTrend?.status==='ready'?pressureTrendText(c):finite(currentPressure)&&finite(futurePressure?.value)?`${futurePressure.value>currentPressure+.2?'↑ Forecast to rise':futurePressure.value<currentPressure-.2?'↓ Forecast to drop':'→ Forecast nearly steady'} · next 3 hours`:'Trend unavailable — waiting for comparable readings.';
 const tiles=[
  ...
  ['pressure','gauge',`${number(currentPressure,1)}<small>mb</small>`,pressureNote],
 ...
 $('metrics').innerHTML=tiles.map(([key,ic,value,note])=>key==='airQuality'
  ? `<button type="button" class="glass metric metric-air-quality" data-air-quality-details aria-expanded="false" aria-controls="air-quality"><span class="metric-title">${smallIcon(ic)}Air Quality<span class="tile-arrow" aria-hidden="true">↗</span></span><span class="metric-value">${value}</span><span class="metric-note">${esc(note)}</span><span class="tile-hint">Show forecast details <span aria-hidden="true">→</span></span></button>`
