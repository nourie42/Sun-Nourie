...
export function pavementHTML(result,feels){
 ...
 return `<figure class="exposure-person pavement-person" id="pavement-content" data-status="${result.status}" aria-label="Estimated hard-surface temperature for dogs right now"><span class="exposure-label">For Pets</span><span class="exposure-alert-slot">${warning?`<span class="pavement-warning" data-risk="${warning.level}" role="status">${warning.label}</span>`:''}</span><svg class="poodle-scene" viewBox="0 0 220 180" role="img" aria-label="A person walking a light brown toy poodle"><image class="poodle-walk" data-outfit="${walker.outfit}" href="/weather-fusion/${walker.asset}" x="0" y="32" width="220" height="146.667"/></svg><figcaption><strong>${value(result.concrete)}</strong><small class="pavement-secondary">Asphalt ${value(result.asphalt)}</small><small>Est. surface · °F</small></figcaption></figure>`;
}
