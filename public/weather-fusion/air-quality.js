...
export function airQualityHTML(forecast){
 ...
 if(!aq||!finite(aq.aqi)){
  return '<p class="aqi-unavailable">Current U.S. AQI data is unavailable. Other weather data is unaffected.</p>';
 }
 ...
 return `<div class="aqi-summary" data-aqi="${esc(band.key)}">
   <div class="aqi-reading"><span>U.S. AQI</span><strong>${Math.round(aq.aqi)}</strong><b>${esc(band.label)}</b></div>
   <div class="aqi-copy"><p>${esc(band.summary)}</p><span>${peak===null?'Next 24-hour peak unavailable':`Next 24-hour peak: <strong>${peak}</strong>`}</span></div>
  </div>
  ...
  <p class="aqi-source">Air-quality model data from Open-Meteo / CAMS. <a href="https://www.airnow.gov/aqi/aqi-basics/" target="_blank" rel="noopener noreferrer">U.S. AQI guidance ↗</a></p>`;
}
