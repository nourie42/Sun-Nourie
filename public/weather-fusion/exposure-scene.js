...
export function exposureScene(sun,daylight=true){
 ...
 return `<svg viewBox="0 0 220 180" role="img" aria-label="${sun?'A smiling person with eyes and hair waving in the sunshine':'A smiling person with eyes and hair waving beneath a tall shade tree'}"${sun&&!daylight?' class="sun-unavailable"':''}>${sun?sunshine:tree}<path d="M13 167H204" stroke="#b6d5d2" stroke-opacity=".4" stroke-width="2"/>${person()}</svg>`;
}
