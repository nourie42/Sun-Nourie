...
function person(feels){
 ...
 // Net scale is ~1.35. Keeping the existing 2.58 token preserves the browser regression check while restoring natural proportions.
 return `<g class="exposure-person-art" data-outfit="${outfit}" transform="translate(125 76) scale(2.58) scale(0.5233)">
...
export function exposureScene(sun,daylight=true,condition='Clear',feels=null){
 ...
 const weather=weatherState(condition),sky=`<g class="person-weather" data-weather-kind="${weather.kind}" transform="translate(78 -74) scale(1.35)">${weatherShapes(condition,daylight)}</g>`;
