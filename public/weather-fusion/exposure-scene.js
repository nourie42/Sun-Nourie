...
function person(feels){
 ...
 return `<g class="exposure-person-art" data-outfit="${outfit}" transform="translate(118 3.8) scale(2.58)">
...
export function exposureScene(sun,daylight=true,condition='Clear',feels=null){
 ...
 const weather=weatherState(condition),sky=`<g class="person-weather" data-weather-kind="${weather.kind}" transform="translate(72 -230) scale(2.2)">${weatherShapes(condition,daylight)}</g>`;
 const treeSky=weather.known?`<g class="shade-weather" transform="translate(72 -230) scale(2.2)">${weatherShapes(condition,daylight)}</g>`:'';
 ...
 return `<svg viewBox="0 -240 220 420" role="img" aria-label="${label}" data-outfit="${outfit}">${sun?sky:tree+treeSky}<path d="M13 167H204" stroke="#b6d5d2" stroke-opacity=".4" stroke-width="2"/>${person(feels)}</svg>`;
}
