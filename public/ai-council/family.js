...
const TOOL_DEFS={
  webSearch:{label:'Web Search',icon:'⌕',description:'Use live online research when answering.'},
...
function normalizeTools(tools){return{webSearch:!!tools?.webSearch,gmail:!!tools?.gmail,browser:!!tools?.browser,shopping:!!tools?.shopping}}
...
function refreshToolStatuses(){
  const web=$('tool-webSearch-status');if(web){web.textContent=configured('perplexity')?'Ready · live research uses Perplexity':'Needs PERPLEXITY_API_KEY before live search can run';web.className=configured('perplexity')?'ready':'warn'}
...
async function unlock(){const record=stored(),pass=els.unlockSecret.value;if(!record)return initLock();try{vault=await openVault(record,pass);secret=pass;vault.bots=(vault.bots||[]).map(bot=>({...bot,tools:normalizeTools(bot.tools)}));vault.chats=vault.chats||{};unlockUi()}catch{err('That secret key is not correct.')}}
function unlockUi(){err('');show(els.lock,false);show(els.workspace,true);renderBots();showBots()}
...
function showBots(){activeBot=null;show(els.botsView,true);show(els.chatView,false);show(els.composer,false);renderBots()}
function openChat(botId){activeBot=vault.bots.find(b=>b.id===botId);if(!activeBot)return;els.chatTitle.textContent=activeBot.name;const tools=toolSummary(activeBot);els.chatRole.textContent=`${activeBot.role||'General expert'} · ${providers.find(p=>p.id===activeBot.provider)?.label||activeBot.provider}${tools?` · ${tools}`:''}`;show(els.botsView,false);show(els.chatView,true);show(els.composer,true);renderMessages();setTimeout(()=>els.prompt.focus(),50)}
