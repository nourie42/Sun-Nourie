(() => {
  ...
  function openDetails(record) {
    ...
    els.detailBody.innerHTML = `
      ...
      <div class="detail-block"><h3>How this location is classified</h3>
      ...
      <div class="detail-block"><h3>Source</h3>
        <p class="empty">${escapeHtml(record.sourceName)}. NAICS qualifies a record for the directory; explicit facility wording determines whether it is a distributor, bulk plant, terminal, heating-oil dealer or propane dealer.</p>
  ...
  function updateSearchButton() {
    ...
    els.searchHelp.textContent = readiness.ready
      ? "Ready to search verified distributors, dealers, bulk plants and terminals. Categories use explicit facility wording."
  ...
  function showAreaTooLarge() {
    ...
    els.results.innerHTML = `<p class="empty"><strong>This map view is too large for a dependable detailed search.</strong><br>Enter a city, state, ZIP code or address above, or zoom to level ${MIN_SEARCH_ZOOM}+.</p>`;
  ...
  async function loadArea({ fitResults = false } = {}) {
    ...
    setStatus("Searching verified distributors, fuel dealers, bulk plants and terminals…", "loading");
    ...
    try {
      ...
      setStatus(
        `${records.length.toLocaleString()} verified locations loaded${suffix}: ${counts.distributor.toLocaleString()} distributors/dealers, ${counts.bulk_plant.toLocaleString()} bulk plants, ${counts.terminal.toLocaleString()} terminals, ${counts.heating_oil.toLocaleString()} heating-oil and ${counts.propane.toLocaleString()} propane.${partial}`,
  ...
  async function findPlace(query) {
    ...
    setLoadingOverlay(true, "Finding your search area…", `Locating ${value} before searching verified distributors.`);
  ...
  els.placeForm.addEventListener("submit", (event) => {
    ...
    const localMatches = records.filter((record) => [
      record.name, record.owner, record.operator,
  ...
  map.on("moveend zoomend", () => {
    ...
    if (!loading) {
      ...
      setStatus(
        readiness.ready
          ? "Map moved—search this area to refresh corrected facility categories."
          : `Zoom to level ${MIN_SEARCH_ZOOM}+ or find a city/state.`,
