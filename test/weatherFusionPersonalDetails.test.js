...
test('production integration and cache-busting cover every new component',()=>{
 ...
 assert.match(app,/const hero = currentHero\(data, day\)/);assert.match(app,/dayGraphHTML\(forecast,index,p\.tonight,now\)/);assert.match(app,/api\('bulletins'/);
 assert.match(html,/Dan's take/);assert.match(html,/Current temperature/);assert.match(html,/Because Apple, Google and Samsung weather suck/);assert.match(html,/personal-details\.css\?v=wpc-mpd-v12/);assert.match(html,/id="bulletin-dialog"/);
 assert.ok(html.indexOf('id="nws-bulletins"')<html.indexOf('class="glass today-panel"'));assert.ok(html.indexOf('id="nws-bulletins"')<html.indexOf('id="hourly-title"'));
