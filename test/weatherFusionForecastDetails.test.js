...
test('changed assets are cache-busted and late briefing responses stay guarded',()=>{
 assert.match(html,/style\.css\?v=experimental-whats-up-v1/);assert.match(html,/forecast-layout\.css\?v=weather-qa-v67/);assert.match(html,/personal-details\.css\?v=wpc-mpd-v12/);assert.match(html,/app\.js\?v=daily-card-balance-v2/);assert.match(html,/weather-polish\.css\?v=daily-align-aqi-color-v3/);assert.match(html,/car-wash\.css\?v=rain-consensus-v41/);assert.match(html,/forecast-cards\.css\?v=weather-qa-v70/);assert.match(html,/scenario-layout\.css\?v=daily-card-confidence-left-v4/);assert.match(html,/risk-outlooks\.css\?v=risk-outlooks-v4/);
 assert.match(app,/dans-take\.js\?v=weather-art-labels-v10/);
 assert.match(app,/bulletins\.js\?v=wpc-mpd-v1/);
 assert.match(app,/experience\.js\?v=feels-floor-wind-v1/);
