...
test('changed assets are cache-busted and late briefing responses stay guarded',()=>{
 assert.match(html,/style\.css\?v=car-wash-icon-back-v46/);assert.match(html,/forecast-layout\.css\?v=weather-art-labels-v10/);assert.match(html,/personal-details\.css\?v=wpc-mpd-v12/);assert.match(html,/app\.js\?v=nws-weight-v60/);assert.match(html,/car-wash\.css\?v=rain-consensus-v41/);assert.match(html,/forecast-cards\.css\?v=remainder-today-v42/);assert.match(html,/scenario-layout\.css\?v=scenario-weather-v29/);
 ...
 assert.match(app,/weather-display\.js\?v=radar-dry-v50/);assert.match(app,/current-temperature\.js\?v=radar-dry-v48/);assert.match(app,/car-wash\.js\?v=radar-now-v47/);
 assert.match(app,/model-explanation\.js\?v=nws-weight-v48/);
