...
test('changed assets are cache-busted and late briefing responses stay guarded',()=>{
 assert.match(html,/style\.css\?v=experimental-whats-up-v1/);assert.match(html,/forecast-layout\.css\?v=weather-qa-v67/);assert.match(html,/personal-details\.css\?v=wpc-mpd-v12/);assert.match(html,/app\.js\?v=nav-cards-restored-daily-v6/);assert.match(html,/weather-polish\.css\?v=nav-cards-restored-daily-v6/);assert.match(html,/car-wash\.css\?v=rain-consensus-v41/);assert.match(html,/forecast-cards\.css\?v=weather-qa-v70/);assert.match(html,/scenario-layout\.css\?v=nav-cards-restored-daily-v6/);assert.match(html,/risk-outlooks\.css\?v=risk-outlooks-v4/);
 assert.match(app,/dans-take\.js\?v=weather-art-labels-v10/);
 assert.match(app,/bulletins\.js\?v=wpc-mpd-v1/);
 assert.match(app,/experience\.js\?v=small-wind-slot-v2/);
 assert.match(app,/personal-details\.js\?v=feels-floor-wind-v1/);
 assert.match(app,/forecast-story\.js\?v=weather-qa-v67/);
 assert.match(app,/weather-display\.js\?v=feels-floor-wind-v1/);assert.match(app,/current-temperature\.js\?v=rain-now-v71/);assert.match(app,/car-wash\.js\?v=full-day-rain-v1/);
 assert.match(app,/model-explanation\.js\?v=weather-qa-v70/);
 assert.match(app,/outlook-details\.js\?v=full-day-rain-v1/);
 for(const module of ['dans-summary','dewpoint-meter'])assert.match(app,new RegExp(`${module}\\.js\\?v=weather-qa-v67`));
