...
test('changed assets are cache-busted and late briefing responses stay guarded',()=>{
 assert.match(html,/style\.css\?v=experimental-whats-up-v1/);assert.match(html,/forecast-layout\.css\?v=weather-qa-v67/);assert.match(html,/personal-details\.css\?v=wpc-mpd-v12/);assert.match(html,/app\.js\?v=weathernext-live-v1/);assert.match(html,/weather-polish\.css\?v=nav-reference-final-v1/);assert.match(html,/car-wash\.css\?v=rain-consensus-v41/);assert.match(html,/forecast-cards\.css\?v=weather-qa-v70/);assert.match(html,/scenario-layout\.css\?v=gust-safe-daily-v1/);assert.match(html,/risk-outlooks\.css\?v=risk-outlooks-v4/);
