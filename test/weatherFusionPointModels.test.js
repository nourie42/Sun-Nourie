...
test('surface danger and uncertainty warnings sit above the walker, with no safe claim',()=>{
 ...
 const html=pavementHTML(result(135,155),99);assert.ok(html.indexOf('Paw burn risk')<html.indexOf('<img'));assert.ok(html.indexOf('For Pets')<html.indexOf('<img'));assert.match(html,/poodle-walk-hot/);assert.doesNotMatch(html,/>Sidewalk</);
});
