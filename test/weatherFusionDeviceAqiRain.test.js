...
test('air quality feed normalizes current US AQI and computes a next-24-hour peak',()=>{
 ...
 assert.match(airQualityHTML({airQuality:out.airQuality}),/U\.S\. AQI/);
 assert.match(airQualityHTML({airQuality:out.airQuality}),/PM2\.5/);
});
...
test('regular and experimental weather share device-location, jump-nav, AQI and 12px readability assets',()=>{
 ...
 assert.match(html,/href="#map-panel">Weather Map/);
 assert.match(html,/href="#dewpoint-gross-meter">Dew Point/);
 assert.match(html,/href="#your-day">Your Day/);
 assert.match(html,/href="#daily-panel">7-Day Forecast/);
