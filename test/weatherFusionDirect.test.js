...
test('hourly rain likelihood scales the NWS share and tiers wet-model points',()=>{
 ...
 assert.equal(deterministicRainSignal(.004),30);
 assert.equal(deterministicRainSignal(.009999),30);
 assert.equal(deterministicRainSignal(.01),30);
 assert.equal(deterministicRainSignal(.016),30);
 assert.equal(deterministicRainSignal(.099999),30);
 ...
 assert.equal(unanimous.value,60);
 assert.equal(unanimous.rawTotal,60.2);
 assert.deepEqual(unanimous.sources.map(source=>source.points),[21.2,30,3,6]);
 ...
 assert.equal(screenshot.value,12);
 const boundary=precipitationLikelihood(7,{sourceValues:{hrrr:0,ecmwf:.004,nbm:.010}});
 assert.equal(boundary.value,12);
 assert.deepEqual(boundary.sourcePoints,{nws:2.8,hrrr:0,ecmwf:3,nbm:6});
 assert.equal(screenshot.weightedValue,11.8);
 assert.deepEqual(screenshot.sourcePoints,{nws:2.8,hrrr:0,ecmwf:3,nbm:6});
...
test('direct models materially change numeric forecast and the source signature',()=>{
 ...
 assert.deepEqual(a.days[0].highBlend.sources.map(s=>[s.id,s.weight]),[['nws',.4],['hrrr',.3],['ecmwf',.1],['nbm',.2]]);
 assert.ok(a.days[0].highBlend.sources.some(s=>s.id==='ecmwf'));
 assert.ok(a.precipitation.value>0);
 ...
 assert.ok(Number.isFinite(a.days[0].rainLikelihood.value));
 assert.ok(Number.isFinite(a.days[0].popDayLikelihood.value));
 assert.ok(Number.isFinite(a.days[0].popNightLikelihood.value));
 assert.equal(a.hours[0].officialPop,testInputs.hourly.periods[0].probabilityOfPrecipitation.value);
 assert.ok(Number.isFinite(a.hours[0].rainLikelihood.value));
