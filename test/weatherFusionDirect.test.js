...
test('hourly rain likelihood is an NWS-anchored weighted average of deterministic QPF support',()=>{
 ...
 assert.equal(mixed.value,22);
 assert.equal(mixed.calibrated,false);
 const reported=precipitationLikelihood(31,{sourceValues:{hrrr:0,ecmwf:.138,nbm:.138}});
 assert.equal(reported.weightedValue,21.7);assert.equal(reported.value,22);
 assert.deepEqual(reported.sourceValues,{nws:31,hrrr:0,ecmwf:31,nbm:31});
 assert.deepEqual(reported.qpfSupport,{hrrr:0,ecmwf:100,nbm:100});
 ...
 assert.equal(lowOfficialAndDryEcmwf.rawValue,4);assert.equal(lowOfficialAndDryEcmwf.value,0);
