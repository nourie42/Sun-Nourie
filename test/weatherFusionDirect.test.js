...
test('hourly rain likelihood gives full model support to measurable rain',()=>{
 ...
 assert.equal(deterministicRainSignal(.099999),100);
 assert.equal(deterministicRainSignal(.1),100);
 assert.equal(deterministicRainSignal(.4),100);
 assert.equal(deterministicRainSignal(.004,{threeHour:.1}),100);
 assert.equal(deterministicRainSignal(.004,{sixHour:.1}),100);
 const unanimous=precipitationLikelihood(53,{sourceValues:{hrrr:.157,ecmwf:.016,nbm:.012}});
 assert.equal(unanimous.value,81);
 assert.equal(unanimous.rawTotal,81.2);
 ...
 assert.deepEqual(boundary.sourcePoints,{nws:2.8,hrrr:0,ecmwf:3,nbm:20});
 assert.equal(screenshot.weightedValue,25.8);
 assert.deepEqual(screenshot.sourcePoints,{nws:2.8,hrrr:0,ecmwf:3,nbm:20});
