...
/* Optional custom traffic endpoint (before DOT) */
async function queryCustomTraffic(lat, lon, address) {
  ...
  if (!tpl) return null;
  const url = tpl.replace("{lat}", encodeURIComponent(lat))
  ...
  try {
    ...
    for (const k of Object.keys(j)) {
      ...
      if (year == null && /(year|date)/.test(lk)) {
        const mt = String(j[k]).match(/20\d{2}/)?.[0]; if (mt) year = +mt;
      }
    }
...
app.post("/estimate", async (req, res) => {
  try {
    ...
    // site
    let geo;
    if (Number.isFinite(siteLat) && Number.isFinite(siteLon)) geo = { lat: Number(siteLat), lon: Number(siteLon), label: address || `${siteLat}, ${siteLon}` };
