...
/* ───────────────────────────────── Config ─────────────────────────────────── */
const CONTACT = process.env.OVERPASS_CONTACT || "FuelEstimator/3.1 (contact: you@example.com)";
const UA = "FuelEstimator/3.1 (+contact: you@example.com)";

const GOOGLE_API_KEY = process.env.GOOGLE_API_KEY || "";
const OPENAI_API_KEY = process.env.OPENAI_API_KEY || "";

// Optional web-search providers for dynamic developments
...
// Optional provider lists (JSON or HTML pages). You can leave these blank.
  ...
  for (const ep of OVERPASS) {
    for (let i=0;i<3;i++){
      try{
        ...
        if (!r.ok || !ct.includes("application/json")) throw new Error(`Overpass ${r.status}: ${txt.slice(0,180)}`);
...
/* ──────────────────────── Dynamic developments (city-aware) ───────────────── */
...
async function exhaustiveDevelopments(addressLabel, lat, lon){
  ...
  // Optional HTML boards, plus dynamic Bing searches and OSM
...
/* ───────────────────────── Google Places (status, ac, rating) ────────────── */
...
/* ───────────────────────── Gallons (baseline ceiling ALWAYS) ─────────────── */
function gallonsWithRules({ aadt, mpds, diesel, compCount, heavyCount, pricePosition, userExtrasMult = 1.0 }) {
  ...
  // Heavy penalties (only specified brands)
  ...
  const base = Math.min(Math.round(capped * priceMult * userExtrasMult), Math.round(baselineMonthly)); // never exceed baseline
  return {
    base,
...
app.post("/estimate", async (req, res) => {
  try {
    ...
    // Developments — dynamic (news, web permits, OSM)
