// server.js — Fuel IQ API v2025-08-29k (fixes + fast, robust endpoints)
...
/* ───────────────── Config / ENV ──────────────── */
const UA = "FuelEstimator/3.2 (+your-app)";
...
async function gptSummary(ctx){
  const sys='Return {"summary":"<text>"} with ~8–12 sentences. Include: AADT method/value; baseline ceiling; competition rule/heavy penalties; pricing position; user adjustments; caps; low/base/high; road context; notable developments.';
  const prompt = `
Inputs:
- Address: ${ctx.address}
...
`.trim();
  ...
  return `AADT ${ctx.aadt} (${ctx.method}); Baseline ceiling active. Competition ${ctx.compCount} (heavy=${ctx.heavyCount}). Pricing ${ctx.pricePosition}. Adjustments: ${ctx.userAdj||"none"}. Result ${ctx.base.toLocaleString()} (range ${ctx.low.toLocaleString()}–${ctx.high.toLocaleString()}). Developments: news(${ctx.devNews||"none"}); permits(${ctx.devPermits||"none"}).`;
}
...
app.post("/estimate", async (req,res)=>{
  try{
    ...
    // Competitors: get 3 mi, then take 1.5 mi for rule & map
    ...
    // User adjustment text (for summary)
    const adjBits=[];
