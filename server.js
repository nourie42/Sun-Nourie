...
const CONTACT = process.env.OVERPASS_CONTACT || "FuelEstimator/1.6 (contact: you@example.com)";
const UA = "FuelEstimator/1.6 (+contact: you@example.com)";
...
function distMiles(lat1, lon1, lat2, lon2) {
  return toMiles(haversine(lat1, lon1, lat2, lon2));
}
...
async function queryNCDOTNearestAADT(lat, lon, radiusMeters = 1609) {
  ...
  const r = await fetchWithTimeout(
    `${NCDOT_AADT_FS}/query?${params.toString()}`,
    { headers: { "User-Agent": UA, Accept: "application/json" } },
    20000
  );
  if (!r.ok) return null;
  ...
  for (const f of feats) {
    ...
    const pairs = Object.keys(attrs)
      ...
      .map((k) => {
        ...
        for (const yk of [
          ...
          const yv = attrs[yk];
          if (yv) {
            const mt = String(yv).match(/20\d{2}/)?.[0];
            if (mt) {
              year = +mt;
              break;
            }
    ...
    const x = f.geometry?.x ?? f.geometry?.longitude,
      y = f.geometry?.y ?? f.geometry?.latitude;
...
/* Optional: your own traffic endpoint (preferred before DOT) */
async function queryCustomTraffic(lat, lon, address) {
  ...
  try {
    ...
    for (const k of Object.keys(j)) {
      ...
      if (value == null && /(aadt|volume|count)/.test(lk) && typeof j[k] === "number" && j[k] > 0)
        value = j[k];
...
async function competitorsWithin1Mile(lat, lon) {
  ...
  try {
    ...
    const d = 0.0145;
    const qb = `[out:json][timeout:25];(node["amenity"="fuel"](${lat - d},${lon - d},${lat + d},${
      lon + d
    });way["amenity"="fuel"](${lat - d},${lon - d},${lat + d},${lon + d}););out center tags;`;
    const b = await overpassQuery(qb);
    elements = b.elements || [];
  }
  const heavy =
    /sheetz|wawa|quik.?trip|(^|\b)qt\b|racetrac|buc-?ee|costco|sam's|bj's|pilot|love's|circle k|speedway|murphy|exxon|shell|bp|chevron|marathon|7-?eleven/i;
  const list = [];
...
async function developments1Mile(lat, lon) {
  ...
  try {
    ...
    const qb = `[out:json][timeout:25];(
      ...
      node["description"~"(?i)(coming soon|proposed|permit|construction|planned)"](${lat - d},${lon - d},${
      lat + d
    },${lon + d});
      way["description"~"(?i)(coming soon|proposed|permit|construction|planned)"](${lat - d},${lon - d},${
      lat + d
    },${lon + d});
    );out center tags;`;
    const b = await overpassQuery(qb);
  ...
  for (const el of elements) {
    ...
    const status = t.construction
      ? "construction"
      : t["proposed:amenity"]
      ? "proposed"
      : t.opening_date
      ? `opening ${t.opening_date}`
      : t.description
      ? t.description
      : "planned?";
    const latc = el.lat ?? el.center?.lat;
  ...
  const seen = new Set(),
    uniq = [];
  for (const d of out) {
    const k = `${d.name}|${Math.round(d.miles * 100)}`;
    if (!seen.has(k)) {
      seen.add(k);
      uniq.push(d);
    }
...
async function roadContext(lat, lon) {
  ...
  try {
    ...
    return { summary: "no data", main: [], side: [], signals: 0, intersections: 0 };
  }
  ...
  const main = ways
    .filter((w) => /^(motorway|trunk|primary|secondary|tertiary)$/.test(w.tags?.highway || ""))
    .map(classify);
  const side = ways
    .filter((w) => /^(residential|service|unclassified)$/.test(w.tags?.highway || ""))
    .map(classify);
  const signals = nodes.filter((n) => n.tags?.highway === "traffic_signals").length;
  const intersections = nodes.filter((n) => /(junction|crossing)/.test(n.tags?.highway || "") || n.tags?.junction)
    .length;
  const rankOrder = { motorway: 6, trunk: 5, primary: 4, secondary: 3, tertiary: 2, residential: 1, service: 0 };
  const dominant = main
    .slice()
    .sort((a, b) => (rankOrder[b.highway] || 0) - (rankOrder[a.highway] || 0) || (b.lanes || 0) - (a.lanes || 0))[0];
  const summary = dominant
    ? `${dominant.highway}${dominant.lanes ? ` ${dominant.lanes} lanes` : ""}${dominant.oneway ? " oneway" : ""}${
        dominant.maxspeed ? ` @ ${dominant.maxspeed}` : ""
      }`
    : "local roads";
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a precise fuel/traffic analyst. Always reply with STRICT JSON (no markdown). Use numeric reasoning and ranges.",
          },
...
async function gptEstimateAADT(ctx) {
  try {
    const prompt = `
Estimate AADT for the described site carefully. Use U.S. norms by highway class, lanes, speed, signals density, and network context.
Return JSON only: {"aadt": <number>, "confidence": <0..1>, "explanation": "<1-3 sentences>"}.
...
async function googleAutocomplete(input) {
  ...
  // 1) predictions
  const au = `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(
    input
  )}&components=country:us&key=${GOOGLE_API_KEY}`;
  const ar = await fetchWithTimeout(au, { headers: { "User-Agent": UA, Accept: "application/json" } }, 15000);
  const atxt = await ar.text();
  if (!ar.ok) return { ok: false, status: `HTTP_${ar.status}`, error: atxt.slice(0, 200), items: [] };
  let aj;
  try {
    aj = JSON.parse(atxt);
  } catch {
    return { ok: false, status: "PARSE_ERROR", error: atxt.slice(0, 200), items: [] };
  }
  if (aj.status !== "OK" && aj.status !== "ZERO_RESULTS")
    return { ok: false, status: aj.status, error: aj.error_message || "No details", items: [] };
  ...
  for (let i = 0; i < n; i++) {
    ...
    try {
      ...
      if (dj.status !== "OK") continue;
      const res = dj.result;
      const loc = res.geometry?.location;
      if (loc && Number.isFinite(loc.lat) && Number.isFinite(loc.lng)) {
        items.push({
          type: "Google",
          display: res.formatted_address || res.name || preds[i].description,
          lat: +loc.lat,
          lon: +loc.lng,
          place_id: res.place_id || pid,
          score: 1.3,
        });
      }
    } catch {
...
app.get("/google/autocomplete", async (req, res) => {
  try {
    ...
    return res.json({ ok: false, status: "EXCEPTION", error: String(e), items: [] });
  }
});

/* ========== Gallons model (with policy caps) ========== */
function gallonsWithRules({ aadt, mpds, diesel, compCount, heavyCount }) {
  // Floor (your earlier structure)
  const floor = aadt * 0.02 * 8 * 30;

  // Competition rule
  let baseMult = 1.0;
  if (compCount >= 3) baseMult = 0.6;
  else if (compCount === 2) baseMult = 0.75;
  // 0–1 => 1.0
  let extraPenalty = 0.0;
  if (heavyCount === 1) extraPenalty = 0.2;
  else if (heavyCount >= 2) extraPenalty = 0.35;
  const compMult = Math.max(0.2, baseMult - extraPenalty); // never below 20%

  // Daily blend → monthly (unconstrained demand)
  const truckShare = 0.10;
  const autos = aadt * (1 - truckShare);
  const trucks = aadt * truckShare;
  const gpd = autos * 0.020 * 10.2 + trucks * 0.012 * 16.0; // gallon/day nominal @ no competition
  const monthlyUncapped = Math.max(gpd * (365 / 12) * compMult, floor);

  // Equipment capacity estimate (legacy cap)
  const capEquip = (mpds * 25 * 10.5 + (diesel || 0) * 25 * 16) * (365 / 12);

  // **Policy caps**:
  // - Hard ceiling: 28,000 gal/MPD/month
  // - Soft band: if demand > 22,000/MPD/month, apply a 10% queuing penalty
  const HARD_CAP_PER_MPD = 28000;
  const SOFT_BAND_PER_MPD = 22000;

  const capPolicyHard = mpds * HARD_CAP_PER_MPD;
  let capped = Math.min(monthlyUncapped, capEquip, capPolicyHard);

  // Queuing penalty if demand pushes above soft band
  const softBandTotal = mpds * SOFT_BAND_PER_MPD;
  if (monthlyUncapped > softBandTotal) {
    capped = Math.round(capped * 0.90); // 10% loss at peaks
  }
  ...
  return {
    ...
    cap: Math.round(Math.min(capEquip, capPolicyHard)),
...
app.post("/estimate", async (req, res) => {
  try {
    ...
    if (!Number.isFinite(MPDS) || MPDS <= 0)
      return res.status(400).json({ error: "Regular MPDs required (>0)" });
    ...
    // GPT AADT
    const gptAADT = await gptEstimateAADT({
    ...
    // Used AADT = average of available components
    const haveDot = Number.isFinite(dotAADT?.value);
    const haveGpt = Number.isFinite(gptAADT?.value);
    const usedAADT =
      haveDot || haveGpt
        ? Math.round(
            ((haveDot ? dotAADT.value : 0) + (haveGpt ? gptAADT.value : 0)) /
              ((haveDot ? 1 : 0) + (haveGpt ? 1 : 0))
          )
        : 10000;

    // Baseline gallons
    const calcBase = gallonsWithRules({
      aadt: usedAADT,
      mpds: MPDS,
      diesel: DIESEL,
      compCount,
    ...
    // Advanced options: multiplier (from extra % only, since you use checkboxes as "extra")
    let userMult = 1.0;
    const breakdown = {};
    ...
    if (extras.length) {
      const extraMult = extras.reduce((m, e) => m * (1 + e.pct / 100), 1.0);
      userMult *= extraMult;
    ...
    // GPT developments (verified vs unverified)
    let devsAI = { items: [], confidence: 0.0, verified: [], unverified: [] };
    try {
      const djson = await gptJSON(
        `List planned/proposed/permit/coming-soon/construction gas stations within ~1 mile of "${
          address || geo.label
        }". Return JSON only: {"items":[{"name":"<string>","status":"<string>","approx_miles":<number>}], "confidence": <0.0-1.0>}`
      );
      if (Array.isArray(djson.items))
        devsAI = { items: djson.items.slice(0, 20), confidence: +(djson.confidence ?? 0), verified: [], unverified: [] };
    } catch {}
    try {
      const BRAND_HINT =
        /gas|fuel|station|convenience|market|mart|travel|truck|sheetz|wawa|racetrac|buc-?ee|costco|sam'?s|bj'?s|pilot|love'?s|circle\s?k|speedway|murphy|exxon|shell|bp|chevron|marathon|7-?eleven|quik.?trip|(^|\b)qt\b/i;
      ...
      for (const it of devsAI.items || []) {
        ...
        const item = {
          ...
          approx_miles: Number.isFinite(miles) ? +miles : null,
        };
    ...
    if (STRICT_AI_DEVS && devsAI.confidence < 0.6) {
      devsAI.items = [];
      devsAI.unverified = [];
    }

    const nearest = competitors[0]?.miles ?? null;
    const rationale = `Competition rule: comps=${compCount}, heavy=${heavyCount} ⇒ multiplier ${(calcBase.compMult * 100) | 0}%. Floor=${calcBase.floor.toLocaleString()} gal/mo. Capacity cap=${calcBase.cap.toLocaleString()} gal/mo. Caps policy: ~${calcBase.caps.soft_per_mpd.toLocaleString()}/MPD (soft) & ${calcBase.caps.hard_per_mpd.toLocaleString()}/MPD (hard). User multiplier=${calc.user_multiplier}.`;

    // GPT summary documenting user changes and method
    let summary = "";
    try {
      const devTextOSM =
        devs.slice(0, 6).map((x) => `${x.name} (${x.status}, ~${x.miles} mi)`).join("; ") || "none";
      const devTextAI =
        (devsAI.verified || [])
          .slice(0, 6)
          .map((x) => `${x.name} (${x.status || "planned"}, ~${x.approx_miles ?? "?"} mi)`)
          .join("; ") || "none";
      ...
      const userAdjList = Object.entries(calc.user_multiplier_breakdown || {})
        .map(([k, v]) =>
          k === "extras" ? `extras: ${v.map((e) => `${e.pct}% (${e.note || "no note"})`).join("; ")}` : `${k}: ×${v}`
        )
        .join("; ") || "none";
...
- Floor (AADT×2%×8×30) and capacity caps policy (soft ~22k/MPD ⇒ −10% queuing; hard 28k/MPD)
- Document ALL user extras/checks (with comments)
Return JSON: {"summary":"<text>"} (no markdown).`;
      const prompt = `
Inputs:
- Address: ${address || geo.label}
- AADT DOT: ${dotAADT?.value ?? "null"} (${dotAADT?.source || "n/a"} ${dotAADT?.year || "n/a"}, ~${
        dotAADT?.distance_mi ?? "?"
      } mi)
- AADT GPT: ${gptAADT?.value ?? "null"} (confidence ${(gptAADT.confidence * 100) | 0}% ; note: ${
        gptAADT.note || "none"
      })
...
- Capacity caps policy: soft ${calcBase.caps.soft_per_mpd}/MPD (−10% queuing if exceeded), hard ${
        calcBase.caps.hard_per_mpd
      }/MPD
