...
function distMiles(a, b, c, d) {
  return toMiles(haversine(a, b, c, d));
}
...
async function queryCustomTraffic(lat, lon, address) {
  ...
  const url = TRAFFIC_URL.replace("{lat}", encodeURIComponent(lat))
  ...
  try {
    ...
    const j = await r.json();
    const aadt =
      +j.aadt || +j.volume || +j.count;
    const year = j.year
      ? +String(j.year).match(/20\d{2}/)?.[0]
      : null;
    if (aadt > 0)
      return { aadt, year, distM: 0, source: "custom" };
  } catch {}
  return null;
}
...
const HEAVY_BRANDS =
  /(sheetz|wawa|race\s?trac|racetrac|buc-?ee'?s|royal\s?farms|quik.?trip|\bqt\b)/i;
  ...
  while (url && tries < 3) {
    ...
    for (const it of items) {
      const name = it.name || "Fuel",
        latc = it.geometry?.location?.lat,
        lonc = it.geometry?.location?.lng;
  ...
  try {
    ...
    return els.map((e) => {
      ...
      const latc = e.lat ?? e.center?.lat,
        lonc = e.lon ?? e.center?.lon;
      const miles =
        Number.isFinite(latc) && Number.isFinite(lonc)
          ? +distMiles(lat, lon, latc, lonc).toFixed(3)
          : null;
      return {
        name: n,
        status: t.proposed ? "proposed" : "construction",
        approx_miles: miles,
        link: null,
        source: "overpass",
      };
...
async function scrapePermitHTML(url) {
  try {
    ...
    const hits = lines
      .filter((L) =>
        /(gas|fuel|convenience|c-store|station|EP\s?Mart)/i.test(L)
      )
      .slice(0, 50);
    return hits.map((h) => ({
      name: h.slice(0, 160),
      status: "permit/agenda",
      approx_miles: null,
      link: url,
      source: url,
    }));
...
async function bingNewsCityCounty(city, county, state) {
  if (!BING_NEWS_KEY)
    return { news: [], web: [], message: "BING_NEWS_KEY missing" };
  const termSets = [
    [
    ...
    [
      ...
      `${city} ${state} zoning gas station`,
    ],
    [
      `${county} ${state} planning commission gas station`,
      `${county} ${state} permit gas station`,
    ],
  ];
  const headers = {
  ...
  for (const terms of termSets) {
    for (const q of terms) {
      try {
        const nu = `${BING_NEWS_ENDPOINT}?q=${encodeURIComponent(
          q
        )}&count=20&freshness=Year`;
  ...
  const ded = (arr) => {
    const seen = new Set(),
      out = [];
    for (const i of arr) {
      const k = `${(i.name || "").toLowerCase()}|${i.link || ""}|${
        i.status || ""
      }`;
      if (seen.has(k)) continue;
      seen.add(k);
  ...
  return {
    news: ded(newsOut).slice(0, 80),
    web: ded(webOut).slice(0, 80),
    message: "ok",
  };
}
...
app.get("/google/searchplace", async (req, res) => {
  try {
    ...
    if (!q) return res.json({ ok: false, status: "BAD_REQUEST" });
    const url = `https://maps.googleapis.com/maps/api/place/textsearch/json?query=${encodeURIComponent(
      q
    )}&type=gas_station&key=${GOOGLE_API_KEY}`;
...
app.get("/google/findplace", async (req, res) => {
  try {
    ...
    const url = `https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=${encodeURIComponent(
      input
    )}&inputtype=textquery&fields=place_id,name,formatted_address,geometry&key=${GOOGLE_API_KEY}`;
    const r = await fetchWithTimeout(url, { headers: { "User-Agent": UA } }, 15000);
    const j = await r.json();
    ...
    res.json({
      ...
      location: cand.geometry?.location || null,
    });
  } catch (e) {
    res.json({ ok: false, status: "EXCEPTION", error: String(e) });
  }
...
app.get("/google/rating", async (req, res) => {
  try {
    ...
    const j = await r.json();
    if (j.status !== "OK")
      return res.json({ ok: false, status: j.status || "ERROR", error: j.error_message || null });
    const g = j.result || {};
    res.json({
      ok: true,
      status: "OK",
      rating: g.rating || null,
      total: g.user_ratings_total || 0,
      name: g.name || null,
...
app.get("/google/rating_by_location", async (req, res) => {
  try {
    ...
    if (!Number.isFinite(lat) || !Number.isFinite(lon))
      return res.json({ ok: false, status: "BAD_REQUEST" });
    const r = await fetchWithTimeout(
      `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${lon}&radius=300&type=gas_station&key=${GOOGLE_API_KEY}`,
      {},
      15000
    );
    const j = await r.json();
    ...
    const d = await fetchWithTimeout(
      `https://maps.googleapis.com/maps/api/place/details/json?place_id=${it.place_id}&fields=rating,user_ratings_total&key=${GOOGLE_API_KEY}`,
      {},
      15000
    );
    ...
    res.json({
      ok: true,
      status: "OK",
      rating: dj.result?.rating || null,
      total: dj.result?.user_ratings_total || 0,
    });
  } catch (e) {
    res.json({ ok: false, status: "EXCEPTION", error: String(e) });
  }
...
function parseMaxspeed(ms) {
  const m = String(ms || "").match(/(\d+)\s*(mph)?/i);
  return m ? +m[1] : null;
}
...
async function roadContext(lat, lon) {
  ...
  let ways = [],
    signals = 0;
  try {
    const wj = await overpassQuery(qWays);
    ways = wj.elements || [];
  } catch {}
  try {
    const sj = await overpassQuery(qSig);
    signals = (sj.elements || []).length;
  } catch {}
  const rows = ways
    .map((w) => {
      const t = w.tags || {};
      ...
      const lanes =
        +t.lanes || +t["lanes:forward"] || +t["lanes:backward"] || null;
      const speed = parseMaxspeed(t.maxspeed);
      const latc = w.center?.lat,
        lonc = w.center?.lon;
      const d =
        Number.isFinite(latc) && Number.isFinite(lonc)
          ? haversine(lat, lon, latc, lonc)
          : null;
      return {
        name,
        highway: hw,
  ...
  const main = rows.slice(0, 3),
    side = rows.slice(3, 8);
  const nice = (r) =>
    [r.name || r.highway, r.maxspeed ? `${r.maxspeed} mph` : null, r.lanes ? `${r.lanes} lanes` : null]
      .filter(Boolean)
      .join(" • ");
  ...
  return {
    summary: [mainLabel, sideLabel].filter(Boolean).join(" — "),
    main,
    side,
    signals,
    intersections,
...
function heuristicAADT(roads) {
  const dom =
    roads?.main?.[0]?.highway ||
    roads?.side?.[0]?.highway ||
    "";
  const lanes = roads?.main?.[0]?.lanes || roads?.side?.[0]?.lanes || 2;
  const speed =
    roads?.main?.[0]?.maxspeed || roads?.side?.[0]?.maxspeed || null;
  let base = 0;
  switch (dom) {
    case "motorway":
      base = 30000;
...
function gallonsWithRules({
  ...
  else if (compCount >= 5) baseMult = 0.5; // NEW: 5+ competitors -> 50%
  ...
  const capEquip =
    mpds * 25 * 10.5 * 24 * (365 / 12) +
    ((diesel || 0) * 25 * 16 * 24) * (365 / 12);
  const SOFT = 22000,
    HARD = 28000;
  const capSoftTotal = mpds * SOFT,
    capHardTotal = mpds * HARD;
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a precise fuel/traffic analyst. Always reply with STRICT JSON (no markdown).",
          },
...
async function gptJSONWithRetry(prompt) {
  ...
  for (const m of models) {
    for (let i = 0; i < 2; i++) {
      try {
        return await gptJSONCore(m, prompt);
      } catch (e) {
        last = e;
        await sleep(400);
      }
...
async function gptSummary(ctx) {
  const sys =
    'Return {"summary":"<text>"} ~8–12 sentences. Include AADT method, baseline ceiling, competition rule & heavy penalties, pricing, user adjustments, caps, LOW/BASE/HIGH, road context, and notable developments.';
  const prompt = `
Address: ${ctx.address}
...
`.trim();
  ...
  return `AADT ${ctx.aadt} (${ctx.method}); competition ${ctx.compCount} (heavy=${ctx.heavyCount}); pricing ${ctx.pricePosition}; adjustments ${ctx.userAdj ||
    "none"}; result ${ctx.low}–${ctx.high} with base ${ctx.base}.`;
}
...
app.post("/estimate", async (req, res) => {
  try {
    ...
    const compCount = competitors.length,
      heavyCount = competitors.filter((c) => c.heavy).length;
...
const roads = await roadContext(geo.lat, geo.lon).catch(() => ({
  ...
  intersections: 0,
    ...
    if (Number.isFinite(overrideVal) && overrideVal > 0) {
      ...
      let stations = [];
      const routeName =
        roads && roads.main && roads.main[0] && roads.main[0].name
          ? roads.main[0].name
          : null;
      if (routeName) {
        try {
          const byRoute = await queryNCDOTNearestAADTByRoute(
            geo.lat,
            geo.lon,
            routeName
          );
          if (byRoute) {
            ...
            stations = await queryNCDOTRouteStations(
              geo.lat,
              geo.lon,
              routeName
            );
      ...
      if (!dotAADT) {
        let sta = await queryCustomTraffic(geo.lat, geo.lon, address).catch(
          () => null
        );
        if (!sta)
          sta = await queryNCDOTNearestAADT(geo.lat, geo.lon, 1609).catch(
            () => null
          );
      ...
      const comps = [];
      if (Number.isFinite(dotAADT))
        comps.push({ v: dotAADT, w: 1.0, l: "DOT" });
      if (Number.isFinite(heur)) comps.push({ v: heur, w: 0.7, l: "HEUR" });
      if (comps.length) {
        const sumW = comps.reduce((s, c) => s + c.w, 0);
        usedAADT = Math.round(
          comps.reduce((s, c) => s + c.v * c.w, 0) /
            Math.max(0.0001, sumW)
        );
    ...
    const extras = (advanced?.extra || [])
      .map((e) => ({
        pct: +e?.pct,
        note: String(e?.note || "").slice(0, 180),
      }))
      .filter((e) => Number.isFinite(e.pct));
    if (extras.length)
      userExtrasMult *= extras.reduce(
        (m, e) => m * (1 + e.pct / 100),
        1.0
      );
    ...
    // Auto low-rating penalty
    const autoLow =
      auto_low_rating === true ||
      (Number.isFinite(client_rating) && client_rating < 4.0);
    ...
    extras.forEach((e) =>
      adjBits.push(`${e.pct > 0 ? "+" : ""}${e.pct}% ${e.note || "adj."}`)
    );
