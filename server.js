// server.js — Fuel IQ API v2025-08-29b
...
function extractCityState(label) {
  ...
  const state =
    parts.length >= 2
      ? parts[parts.length - 2].match(/[A-Z]{2}/)?.[0] || ""
      : "";
  const city =
...
async function queryNCDOTNearestAADT(lat, lon, radiusMeters = 1609) {
  ...
  rows.sort(
    (A, B) => (B.year || 0) - (A.year || 0) || B.aadt - A.aadt || A.distM - B.distM
  );
  return rows[0];
}
    ...
    for (let i = 0; i < 3; i++) {
      try {
        ...
        if (!r.ok || !ct.includes("application/json"))
          throw new Error(`Overpass ${r.status}: ${txt.slice(0, 160)}`);
...
async function googleNearbyGasStations(lat, lon, radiusMeters = 2414) {
  ...
  while (url && tries < 3) {
    ...
    for (const it of results) {
      ...
      const latc = it.geometry?.location?.lat,
...
async function roadContext(lat, lon) {
  ...
  try {
    ...
    const main = ways
      .filter((w) =>
        /^(motorway|trunk|primary|secondary|tertiary)$/i.test(w.tags?.highway || "")
      )
      .map(classify);
    const side = ways
      .filter((w) =>
        /^(residential|service|unclassified)$/i.test(w.tags?.highway || "")
      )
      .map(classify);
    const signals = nodes.filter((n) => n.tags?.highway === "traffic_signals").length;
    const intersections = nodes.filter(
      (n) => /(junction|crossing)/.test(n.tags?.highway || "") || n.tags?.junction
    ).length;
    ...
    const dom = main
      ...
      .sort(
        (a, b) =>
          (rank[b.highway] || 0) - (rank[a.highway] || 0) || (b.lanes || 0) - (a.lanes || 0)
      )[0];
    const summary = dom
      ? `${dom.highway}${dom.lanes ? ` ${dom.lanes} lanes` : ``}${
          dom.oneway ? " oneway" : ""
        }${dom.maxspeed ? ` @ ${dom.maxspeed}` : ""}`
      : "local roads";
...
function heuristicAADT(roads) {
  ...
  const speed = parseMaxspeed(
    roads?.main?.[0]?.maxspeed || roads?.side?.[0]?.maxspeed
  );
  let base = 0;
  switch (dom) {
    case "motorway":
...
async function gptSummary(ctx) {
  const sys =
    'Return {"summary":"<text>"} with 8–12 concise sentences covering: AADT method/values, baseline ceiling (AADT×2%×8×30), road layout, competition rule (0=100%, 1=75%, 2–4=60% with heavy penalty 20–35%), pricing position (±10%), caps (22–28k/MPD/mo), and developments (news/permits/OSM).';
  const prompt = `
Inputs:
- Address: ${ctx.address}
...
async function scrapePermitHTML(url) {
  try {
    ...
    const hits = lines
      .filter((L) => /(gas|fuel|convenience|c-store|station|EP\s?Mart)/i.test(L))
      .slice(0, 40);
    return hits.map((h) => ({
      name: h.slice(0, 160),
...
async function exhaustiveDevelopments(addressLabel, lat, lon) {
  ...
  const [newsJson, permitsJson] = await Promise.all([
    Promise.all(NEWS_URLS.map((t) => queryExternalJSON(t, base).catch(() => []))).then((a) =>
      a.flat()
    ),
    Promise.all(PERMIT_URLS.map((t) => queryExternalJSON(t, base).catch(() => []))).then(
      (a) => a.flat()
    ),
  ]);

  // Optional HTML boards + dynamic Bing + OSM
  const [permHtml, newsBing, webPermits, osmDev] = await Promise.all([
    Promise.all(PERMIT_HTML_URLS.map((u) => scrapePermitHTML(u).catch(() => []))).then((a) =>
      a.flat()
    ),
...
async function googleAutocomplete(input) {
  if (!GOOGLE_API_KEY)
    return { ok: false, status: "MISSING_KEY", error: "GOOGLE_API_KEY not set", items: [] };
  ...
  if (!ar.ok) return { ok: false, status: `HTTP_${ar.status}`, error: at.slice(0, 200), items: [] };
  let aj;
  try {
    aj = JSON.parse(at);
  } catch {
    return { ok: false, status: "PARSE_ERROR", error: at.slice(0, 200), items: [] };
  }
...
function gallonsWithRules({
  ...
  const base = Math.min(
    Math.round(capped * priceMult * userExtrasMult),
    Math.round(baselineMonthly)
  );
...
app.post("/estimate", async (req, res) => {
  try {
    ...
    if (Number.isFinite(overrideVal) && overrideVal > 0) {
      ...
      const comps = [];
      if (Number.isFinite(dotAADT)) comps.push({ v: dotAADT, w: 1.0, label: "DOT" });
      if (Number.isFinite(heur)) comps.push({ v: heur, w: 0.7, label: "HEUR" });
      if (comps.length) {
        const sumW = comps.reduce((s, c) => s + c.w, 0);
        usedAADT = Math.round(
          comps.reduce((s, c) => s + c.v * c.w, 0) / Math.max(0.0001, sumW)
        );
    ...
    if (extras.length)
      userExtrasMult *= extras.reduce((m, e) => m * (1 + e.pct / 100), 1.0);
    ...
    return res.json({
      ...
      year2: calc.year2,
      year3: calc.year3,
      inputs: { mpds: MPDS, diesel: DIESEL, aadt_used: usedAADT, price_position: pricePosition },
      competition: {
        count: compCount,
