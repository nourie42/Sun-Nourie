...
/* ========== Overpass + Google Nearby (competition) ========== */
const OVERPASS=[
    ...
    try { return (await overpassQuery(q)).elements || []; }
      ...
      const qb=`[out:json][timeout:25];
        ( node["amenity"="fuel"](${lat-pad},${lon-pad},${lat+pad},${lon+pad});
          way["amenity"="fuel"](${lat-pad},${lon-pad},${lat+pad},${lon+pad}); ); out center tags;`;
...
async function queryProviderList(urls, ctx){
  ...
  // de-dupe by (name ~and~ rounded distance)
...
/* ========== Gallons model (baseline ceiling) ========== */
function gallonsWithRules({ aadt, mpds, diesel, compCount, heavyCount, allowAboveBaseline, pricePosition, userExtrasMult = 1.0 }) {
  ...
  // Heavy penalties
  let extraPenalty=0; if(heavyCount===1) extraPenalty=0.20; else if(heavyCount>=2) extraPenalty=0.35;
  const compMult=Math.max(0.20, baseMult - extraPenalty);

  // Demand (only allowed to exceed baseline if user opted in)
  ...
  // Pricing position: below=+10%, inline=0, above=−10%
  let priceMult = 1.0;
...
app.post("/estimate", async (req,res)=>{
  try{
    ...
    // Exhaustive developments (OSM, plus external providers)
    const devs = []; // OSM developments are rare/spotty; if desired you can add an Overpass 'construction/proposed' query here too.
    ...
    const calc=gallonsWithRules({
      ...
      compCount, heavyCount,
      allowAboveBaseline,
      pricePosition,
      userExtrasMult
    });

    // GPT-verified developments (optional; keep strict)
    ...
    try{
      ...
      for(const it of (devsAI.items||[])){
        ...
        const match = [...devs, ...devNews, ...devPermits].some(os => similar(os.name, name) &&
    ...
    // Rationale + summary (guaranteed non-empty)
